"""
algo_mode_duo.py — Mode Duo (2 chevaux) avec filtre concordance
"""
...
def render_duo(df, courses_avec, courses_sans, date_start, date_end,
    ...
    if courses_avec:
        ...
        for cid in courses_avec:
            ...
            st_f['total'] += 1
            cg_f = sf.issubset(top2); cp_f = sf.issubset(top3)
            ...
            for sx, nums in [(st_ib, sib), (st_hyb, sh)]:
                sx['n'] += 1
                if nums.issubset(top2): sx['cg'] += 1
                if nums.issubset(top3): sx['cp'] += 1

            arrivee = get_arrivee(df_c)
            cg_ib = sib.issubset(top2); cp_ib = sib.issubset(top3)
            cg_h = sh.issubset(top2); cp_h = sh.issubset(top3)

            conf_icon = "🟢" if unanime else ("🟡" if concordance >= 4 else "🔴")

            rows_export.append({
                'Course': cid, 'Jouee': '✅' if course_jouee else '⏭️',
                'Conf': conf_icon, 'Concordance': concordance, 'Unanime': '✅' if unanime else '',
                'F_N1': safe_num(duo_f, 0), 'F_N2': safe_num(duo_f, 1),
                'F_CG': "OUI" if cg_f else "NON", 'F_CP': "OUI" if cp_f else "NON",
                'IB_CG': "OUI" if cg_ib else "NON", 'IB_CP': "OUI" if cp_ib else "NON",
                'H_CG': "OUI" if cg_h else "NON", 'H_CP': "OUI" if cp_h else "NON",
                'Arrivee': arrivee or ""
            })
        ...
        if filtre_confiance_on:
            ...
            txt = "Unanimité F=IA=H" if filtre_unanime else f"Concordance ≥ {seuil_concordance}"
            st.info(f"🎯 **Filtre actif** : {st_f['n']} jouées / {st_f['total']} total ({st_f['skip']} skip = {100-pct}%) — {txt}")

        with st.container(border=True):
            k1, k2, k3, k4, k5, k6 = st.columns(6)
            t_f = max(st_f['n'], 1); t_all = max(st_ib['n'], 1)
        ...
        for row in courses_jouees:
            ...
            duo_ib = df_c.nsmallest(2, 'IA_Borda_Rank') if 'IA_Borda_Rank' in df_c.columns else df_c.head(2)
            duo_hyb = df_c.nlargest(2, 'HYBRIDE')
            cg_f = row['F_CG'] == "OUI"; cp_f = row['F_CP'] == "OUI"
        ...
        if courses_skippees and filtre_confiance_on:
            with st.expander(f"⏭️ Courses skippées ({len(courses_skippees)})", expanded=False):
                for row in courses_skippees:
                    ...
                    icon = "🥇" if cg_f else ("✅" if cp_f else "❌")
                    nlist = [int(r['Numero']) for _, r in duo_f.iterrows()]
                    st.caption(f"{icon} {cid} — F: {colored_nums(nlist, top3)} — Conc: {row['Concordance']} — {row['Arrivee']}")
    ...
    if courses_sans:
        ...
        for cid in courses_sans:
            ...
            concordance, detail = calculer_confiance_duo(df_c, 'SCORE')
            unanime = detail.get('unanime', False)
            jouable = (unanime if filtre_unanime else concordance >= seuil_concordance) if filtre_confiance_on else True
            conf_icon = "🟢" if unanime else ("🟡" if concordance >= 4 else "🔴")
            with st.container(border=True):
                status = "🎯 JOUER" if jouable else "⏭️ SKIP"
                st.write(f"**{conf_icon} {cid}** — {status} — Conc: {concordance} {'✅ Unanime' if unanime else ''}")
                c1, c2 = st.columns(2)
                with c1: st.success(f"**🎯 F** {nums_str(duo_f)}")
                with c2: st.info(f"**⚡ H** {nums_str(duo_hyb)}")

    if courses_avec:
        st.download_button("📥 CSV Duo", pd.DataFrame(rows_export).to_csv(index=False, sep=';').encode('utf-8'),
                           f"export_duo_{date_start}_{date_end}.csv", "text/csv", use_container_width=True)